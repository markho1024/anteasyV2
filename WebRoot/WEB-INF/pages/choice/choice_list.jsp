...
<script type="text/javascript">
	$(function() {
		$("#navAdd").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/Chc_add.action";
						});
	});
</script>
...
<script type="text/javascript">
	$(function() {
		$("#navReset").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/Chc_listAll.action?soPick=&startDate=&endDate=";
		});
...
<script type="text/javascript">
	function doDel(id, name) {
		$.messager.confirm('删除确认','你确认删除" ' + name + ' "吗?',function(bt) {
			if (bt) {
				window.location.href = "<%=request.getContextPath()%>/Chc_del?pick.pickID="+id;
			};
		});
	  }
</script>
...
<body>
	...
	<div id="context">
		...
		<div id="rightpane" class="right">
			...
			<div class="navSearch">
				<form action="Chc_query" id="navForm">
					<fieldset>
						<legend>
			...
			<div class="resPage">
				...
				<s:else>
					<a
						href="<%=request.getContextPath()%>/Chc_listAll.action?currentPage=1">第一页</a>
					<a
						href="<%=request.getContextPath()%>/Chc_listAll.action?currentPage=${pageBean.currentPage-1}">上一页</a>
				</s:else>

				<s:if test="%{pageBean.currentPage !=pageBean.totalPage}">
					<a
						href="<%=request.getContextPath()%>/Chc_listAll.action?currentPage=${pageBean.currentPage+1}">下一页</a>
					<a
						href="<%=request.getContextPath()%>/Chc_listAll.action?currentPage=${pageBean.totalPage}">最后一页</a>
				</s:if>
