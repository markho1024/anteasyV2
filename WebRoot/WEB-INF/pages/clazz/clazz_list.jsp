...
<script type="text/javascript">
	$(function() {
		$("#navAdd").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/Clz_add.action";
						});
	});
</script>
...
<script type="text/javascript">
	$(function() {
		$("#navReset").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/Clz_listAll.action?soClazz=&startDate=&endDate=";
		});
	...
	$(function() {
		$("#COLLEGE_ID_S").combotree({
			...
			onChange:function(){
			        ...
			        $("#COLLEGE_ID").val(voc);
					var url= "./MjrJ_comboP?jsonType=all&jsonValue="+voc;
...
<script type="text/javascript">
	function doDel(id, name) {
		$.messager.confirm('删除确认','你确认删除" ' + name + ' "吗?',function(bt) {
			if (bt) {
				window.location.href = "<%=request.getContextPath()%>/Clz_del?clazz.clazzID="+id;
							};
						});
	}
</script>
...
<script type="text/javascript">
	function doOff(id, name) {
		$.messager.confirm('取消确认','你确认取消" ' + name + ' "吗?',function(bt) {
			if (bt) {
				window.location.href = "<%=request.getContextPath()%>/Clz_off?id="+id+"&offID=91";
							};
						});
	}
</script>
 ...
 <div id="page">
	...
	<div id="context">
		...
		<div id="rightpane" class="right">
			...
			<div class="navSearch">
				<form action="Clz_query" id="navForm">
					<fieldset>
						<legend>
							...
							<tr>
								...
								<td><p>
										<input type="text" id="COLLEGE_ID_S" 
											value="${soClazz.collegeID}" />
									</p>
								</td>
								<th>所属院系号</th>
								<td><p>
										<input type="text" id="COLLEGE_ID" name="soClazz.collegeID"
											value="${soClazz.collegeID}" />
									</p>
									<p class="errs"></p>
								</td>
					...
					<tbody>
						<s:iterator value="pageBean.list" status="st" id="bean">
							<tr <s:if test="!#st.odd">class="trodd"</s:if>>
								...
								<td align="center"><a:map type="collegeName" name="college"
										code="#bean.collegeID" /></td>
								...
								<td align="center"><a:dict type="status"
										code="#bean.status" /></td>
								<td align="center"><a
									href="<%=request.getContextPath()%>/Clz_load.action?id=${bean.clazzID}">查看</a>
									<a
									href="<%=request.getContextPath()%>/Clz_edit.action?id=${bean.clazzID}">修改</a>
			...
			<div class="resPage">
				...
				<s:else>
					<a
						href="<%=request.getContextPath()%>/Clz_listAll.action?currentPage=1">第一页</a>
					<a
						href="<%=request.getContextPath()%>/Clz_listAll.action?currentPage=${pageBean.currentPage-1}">上一页</a>
				</s:else>

				<s:if test="%{pageBean.currentPage !=pageBean.totalPage}">
					<a
						href="<%=request.getContextPath()%>/Clz_listAll.action?currentPage=${pageBean.currentPage+1}">下一页</a>
					<a  href="<%=request.getContextPath()%>/Clz_listAll.action?currentPage=${pageBean.totalPage}">最后一页</a>
				</s:if>
