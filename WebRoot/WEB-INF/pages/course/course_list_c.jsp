...
<script type="text/javascript">
	$(function() {
		$("#navAdd").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/CrsE_addC.action";
						});
	});
</script>
...
<script type="text/javascript">
	$(function() {
		$("#navReset").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/CrsE_listC.action?soCourse=&startDate=&endDate=";
		});
...
<script type="text/javascript">
	function doDel(id, name) {
		$.messager.confirm('删除确认','你确认删除" ' + name + ' "吗?',function(bt) {
			if (bt) {
				window.location.href = "<%=request.getContextPath()%>/CrsE_delC?course.id="+id;
			};
		});
	  }
</script>
...
<script type="text/javascript">
	function doOff(id, name) {
		$.messager.confirm('取消确认','你确认取消" ' + name + ' "吗?',function(bt) {
			if (bt) {
				window.location.href = "<%=request.getContextPath()%>/CrsE_offC?id="+id+"&offID=91";
			};
		});
	  }
</script>
 ...
 <div id="page">
	...
	<div id="context">
		...
		<div id="rightpane" class="right">
			...
			<div class="navSearch">
				<form action="CrsE_listC" id="navForm">
					<fieldset>
						<legend>
						...
						<s:iterator value="pageBean.list" status="st" id="bean">
							<tr <s:if test="!#st.odd">class="trodd"</s:if>>
								...
								<td><a:dict type="status"
										code="#bean.status" /></td>
								<td><a
									href="<%=request.getContextPath()%>/CrsE_loadC.action?id=${bean.courseID}">查看</a>
									<a
									href="<%=request.getContextPath()%>/CrsE_editC.action?id=${bean.courseID}">修改</a>
			...
			<div class="resPage">
				...
				<s:else>
					<a
						href="<%=request.getContextPath()%>/CrsE_listC.action?currentPage=1">第一页</a>
					<a
						href="<%=request.getContextPath()%>/CrsE_listC.action?currentPage=${pageBean.currentPage-1}">上一页</a>
				</s:else>

				<s:if test="%{pageBean.currentPage !=pageBean.totalPage}">
					<a
						href="<%=request.getContextPath()%>/CrsE_listC.action?currentPage=${pageBean.currentPage+1}">下一页</a>
					<a
						href="<%=request.getContextPath()%>/CrsE_listC.action?currentPage=${pageBean.totalPage}">最后一页</a>
				</s:if>
