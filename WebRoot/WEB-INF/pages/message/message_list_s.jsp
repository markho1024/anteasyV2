...
<script type="text/javascript">
	$(function() {
		$("#navReset").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/MsgE_listS.action?soMessage=&startDate=&endDate=";
		});
...
<script type="text/javascript">
	function doOff(id, name) {
		$.messager.confirm('删除确认','你确认删除" ' + name + ' "吗?',function(bt) {
			if (bt) {
				window.location.href = "<%=request.getContextPath()%>/Msg_off?message.id="+id+"&offID=9";
			};
		});
	  }
</script>
 ...
 <div id="page">
	...
	<div id="context">
		...
		<div id="rightpane" class="right">
			...
			<div class="navSearch">
				<form action="MsgE_listS" id="navForm">
					<fieldset>
						<legend>
					...
					<tbody>
						<s:iterator value="pageBean.list" status="st" id="bean">
							<tr <s:if test="!#st.odd">class="trodd"</s:if>>
								...
								<td><a
									href="<%=request.getContextPath()%>/MsgE_look.action?id=${bean.messageID}">查看</a>
