...
<script type="text/javascript">
	$(function() {
		$("#navAdd").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/Opn_add.action";
						});
	});
</script>
...
<script type="text/javascript">
	$(function() {
		$("#navReset").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/Opn_listAll.action?soOpen=&startDate=&endDate=&lowCredit=&highCredit=";
		});
...
<script type="text/javascript">
	function doDel(id, name) {
		$.messager.confirm('删除确认','你确认删除" ' + name + ' "吗?',function(bt) {
			if (bt) {
				window.location.href = "<%=request.getContextPath()%>/Opn_del?open.id="+id;
			};
		});
	  }
</script>
...
<script type="text/javascript">
	function doOff(id, name) {
		$.messager.confirm('取消确认','你确认取消" ' + name + ' "吗?',function(bt) {
			if (bt) {
				window.location.href = "<%=request.getContextPath()%>/Opn_off?id="+id+"&offID=91";
			};
		});
	  }
</script>
	...
	<div id="page">
		...
		<div id="context">
			...
			<div id="rightpane" class="right">
				...
				<div class="navSearch">
					<form action="Opn_query" id="navForm">
						<fieldset>
							<legend>
							...
							<table cellpadding="0" cellspacing="0" class="navSearch"
								...
								<tr>
									<th>上课星期</th>
									<td><p>
											<input type="text" id="TABLE_WEEK" name="soOpen.tableWeek"
						...
						<tbody>
							<s:iterator value="pageBean.list" status="st" id="bean">
								<tr <s:if test="!#st.odd">class="trodd"</s:if>>
									...
									<td><s:property value="#bean.orderID" />
									</td>
									<td><a:map type="courseName" name="course"
											code="#bean.courseID" />
									</td>
									<td><a:map type="teacherName" name="teacher"
											code="#bean.teacherID" />
									</td>
									<td><a:dict type="course_type" code="#bean.courseType" />
									</td>
									<td><a:dict type="score_kind" code="#bean.scoreKind" />
									</td>

									<td><s:property value="#bean.credit" />
									</td>
									<td><s:property value="#bean.tableWeek" />-<s:property
											value="#bean.tableDay" />
									</td>
									<td><a:dict type="status" code="#bean.status" />
									<td><a
										href="<%=request.getContextPath()%>/Opn_load.action?id=${bean.openID}">查看</a>
										<a
										href="<%=request.getContextPath()%>/Opn_edit.action?id=${bean.openID}">修改</a>
										<!-- 	<a href="javascript:doDel('<s:property value="#bean.openID"/>','');">删除</a>  -->
										<s:if test="#bean.status==1">
											<a
												href="javascript:doOff('<s:property value="#bean.openID"/>','<s:property value="#bean.openID"/>');">取消</a>
										</s:if></td>
								</tr>
							</s:iterator>
						</tbody>
