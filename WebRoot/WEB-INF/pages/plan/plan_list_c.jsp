...
<script type="text/javascript">
	$(function() {
		$("#navAdd").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/PlnE_addC.action";
						});
	});
</script>
...
<script type="text/javascript">
	$(function() {
		$("#navReset").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/PlnE_listC.action?soPlan=&startDate=&endDate=";
		});
...
<script type="text/javascript">
	function doDel(id, name) {
		$.messager.confirm('删除确认','你确认删除" ' + name + ' "吗?',function(bt) {
			if (bt) {
				window.location.href = "<%=request.getContextPath()%>/PlnE_delC?plan.id="+id;
							};
						});
	}
</script>
<script type="text/javascript">
	function doOff(id, name) {
		$.messager.confirm('取消确认','你确认取消" ' + name + ' "吗?',function(bt) {
			if (bt) {
				window.location.href = "<%=request.getContextPath()%>/PlnE_offC?plan.id="+id+"&offID=91";
							};
						});
	}
</script>
 ...
 <div id="page">
	...
	<div id="context">
		...
		<div id="rightpane" class="right">
			...
			<div class="navSearch">
				<form action="PlnE_listC" id="navForm">
					<fieldset>
						<legend>
					...
					<tbody>
						<s:iterator value="pageBean.list" status="st" id="bean">
							<tr <s:if test="!#st.odd">class="trodd"</s:if>>
								...
								<td><a
									href="<%=request.getContextPath()%>/PlnE_loadC.action?id=${bean.planID}">查看</a>
									<a
									href="<%=request.getContextPath()%>/PlnE_editC.action?id=${bean.planID}">修改</a>
			...
			<div class="resPage">
				...
				<s:else>
					<a
						href="<%=request.getContextPath()%>/PlnE_listC.action?currentPage=1">第一页</a>
					<a
						href="<%=request.getContextPath()%>/PlnE_listC.action?currentPage=${pageBean.currentPage-1}">上一页</a>
				</s:else>

				<s:if test="%{pageBean.currentPage !=pageBean.totalPage}">
					<a
						href="<%=request.getContextPath()%>/PlnE_listC.action?currentPage=${pageBean.currentPage+1}">下一页</a>
					<a
						href="<%=request.getContextPath()%>/PlnE_listC.action?currentPage=${pageBean.totalPage}">最后一页</a>
				</s:if>
