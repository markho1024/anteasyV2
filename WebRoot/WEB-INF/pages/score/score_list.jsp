...
<script type="text/javascript">
	$(function() {
		$("#navAdd").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/Scr_add.action";
						});
	});
</script>
...
<script type="text/javascript">
	$(function() {
		$("#navReset").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/Scr_listAll.action?soPick=&isPass=&soOpen=&startDate=&endDate=";
		});
...
<script type="text/javascript">
	function doDel(id, name) {
		$.messager.confirm('删除确认','你确认删除" ' + name + ' "吗?',function(bt) {
			if (bt) {
				window.location.href = "<%=request.getContextPath()%>/Scr_del?pick.id="+id;
			};
		});
	  }
</script>
...
<script type="text/javascript">
	function doOff(id, name) {
		$.messager.confirm('取消确认','你确认取消" ' + name + ' "吗?',function(bt) {
			if (bt) {
				window.location.href = "<%=request.getContextPath()%>Scr_off?pick.id="+ id + "&offID=91";
							}
							;
						});
	}
 ...
 <div id="page">
	...
	<div id="context">
		...
		<div id="rightpane" class="right">
			...
			<div class="navSearch">
				<form action="Scr_query" id="navForm">
					<fieldset>
						<legend>
					...
					<tbody>
						<s:iterator value="pageBean.list" status="st" id="bean">
							<tr <s:if test="!#st.odd">class="trodd"</s:if>>
								...
								<td><a
									href="<%=request.getContextPath()%>/Scr_load.action?id=${bean.pickID}">查看</a>
									<a
									href="<%=request.getContextPath()%>/Scr_edit.action?id=${bean.pickID}">修改</a>
								    ...
								    <a href="javascript:doOff('<s:property value="#bean.pickID"/>','<s:property value="#bean.pickID"/>');">取消</a> 
										...
										<a
											href="<%=request.getContextPath()%>/Scr_log.action?id=${bean.pickID}">录入成绩</a>
									</s:if></td>
							</tr>
						</s:iterator>
					</tbody>
			...
			<div class="resPage">
				...
				<s:if test="%{pageBean.currentPage == 1}">第一页 上一页 </s:if>
					...
					<s:else>
						<a
							href="<%=request.getContextPath()%>/Scr_listAll.action?currentPage=1">第一页</a>
						<a
							href="<%=request.getContextPath()%>/Scr_listAll.action?currentPage=${pageBean.currentPage-1}">上一页</a>
					</s:else>

					<s:if test="%{pageBean.currentPage !=pageBean.totalPage}">
						<a
							href="<%=request.getContextPath()%>/Scr_listAll.action?currentPage=${pageBean.currentPage+1}">下一页</a>
						<a
							href="<%=request.getContextPath()%>/Scr_listAll.action?currentPage=${pageBean.totalPage}">最后一页</a>
					</s:if>
