...
<script type="text/javascript">
	$(function() {
		$("#navAdd").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/Std_add.action";
						});
	});
</script>
...
<script type="text/javascript">
	$(function() {
		$("#navReset").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/Std_listAll.action?soStudent=";
		});
...
<script type="text/javascript">
	function doDel(id, name) {
		$.messager.confirm('删除确认','你确认删除" ' + name + ' "吗?',function(bt) {
			if (bt) {
				window.location.href = "<%=request.getContextPath()%>/Std_del?student.id="+id;
			};
		});
	  }
</script>
...
<script type="text/javascript">
	function doOff(id, name) {
		$.messager.confirm('退学确认','你确认" ' + name + ' "退学吗?',function(bt) {
			if (bt) {
				window.location.href = "<%=request.getContextPath()%> /Std_off?student.id=" + id + "&offID=96";
							}
							;
						});
	}
 ...
 <div id="page">
	...
	<div id="context">
		...
		<div id="rightpane" class="right">
			...
			<div class="navSearch">
				<form action="Std_query" id="navForm">
					<fieldset>
						<legend>
							...
							<tr>
								...
								<td><p>
										<input type="text" id="COLLEGE_ID_S"
											value="${soStudent.collegeID}" />
									</p></td>
								<th>所属院系号</th>
								<td><p>
										<input type="text" id="COLLEGE_ID" name="soStudent.collegeID"
											value="${soStudent.collegeID}" />
									</p>
									<p id="error">
									</p>
	 ...
	 -->
		 ...
		 -->
								...
								<td><a
									href="<%=request.getContextPath()%>/Std_load.action?id=${bean.studentID}">查看</a>
									<a
									href="<%=request.getContextPath()%>/Std_edit.action?id=${bean.studentID}">修改</a>
									<!-- <a
			...
			<div class="resPage">
				...
				<s:if test="%{pageBean.currentPage == 1}">第一页 上一页 </s:if>
					...
					<s:else>
						<a
							href="<%=request.getContextPath()%>/Std_listAll.action?currentPage=1">第一页</a>
						<a
							href="<%=request.getContextPath()%>/Std_listAll.action?currentPage=${pageBean.currentPage-1}">上一页</a>
					</s:else>

					<s:if test="%{pageBean.currentPage !=pageBean.totalPage}">
						<a
							href="<%=request.getContextPath()%>/Std_listAll.action?currentPage=${pageBean.currentPage+1}">下一页</a>
						<a
							href="<%=request.getContextPath()%>/Std_listAll.action?currentPage=${pageBean.totalPage}">最后一页</a>
					</s:if>
