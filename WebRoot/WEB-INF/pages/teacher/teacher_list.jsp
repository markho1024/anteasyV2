...
<script type="text/javascript">
	$(function() {
		$("#navAdd").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/Tcr_add.action";
						});
	});
</script>
...
<script type="text/javascript">
	$(function() {
		$("#navReset").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/Tcr_listAll.action?soTeacher=&startDate=&endDate=";
		});
...
<script type="text/javascript">
	function doDel(id, name) {
		$.messager.confirm('删除确认','你确认删除" ' + name + ' "吗?',function(bt) {
			if (bt) {
				window.location.href = "<%=request.getContextPath()%>/Tcr_del?teacher.teacherID="+id;
							}
							;
						});
	}
...
<script type="text/javascript">
	function doOff(id, name) {
		$.messager.confirm('离职确认','你确认" ' + name + ' "离职吗?',function(bt) {
			if (bt) {
				window.location.href = "<%=request.getContextPath()%>/Tcr_off?id="+ id + "&offID=95";
							}
							;
						});
	}
 ...
 <div id="page">
	...
	<div id="context">
		...
		<div id="rightpane" class="right">
			...
			<div class="navSearch">
				<form action="Tcr_query" id="navForm">
					<fieldset>
						<legend>
                             ...
                             <tr>
								...
								<td><p>
										<input type="text" id="COLLEGE_ID_S"
											value="${soTeacher.collegeID}" />
									</p></td>
								<th>所属院系号</th>
								<td><p>
										<input type="text" id="COLLEGE_ID" name="soTeacher.collegeID"
											value="${soTeacher.collegeID}" />
									</p>
									<p class="errs"></p></td>
							</tr>
					...
					<tbody>
						<s:iterator value="pageBean.list" status="st" id="bean">
							<tr <s:if test="!#st.odd">class="trodd"</s:if>>
								...
								<td><a
									href="<%=request.getContextPath()%>/Tcr_load.action?id=${bean.teacherID}">查看</a>
									<a
									href="<%=request.getContextPath()%>/Tcr_edit.action?id=${bean.teacherID}">修改</a>
			...
			<div class="resPage">
				...
				<s:if test="%{pageBean.currentPage == 1}">第一页 上一页 </s:if>
					...
					<s:else>
						<a
							href="<%=request.getContextPath()%>/Tcr_listAll.action?currentPage=1">第一页</a>
						<a
							href="<%=request.getContextPath()%>/Tcr_listAll.action?currentPage=${pageBean.currentPage-1}">上一页</a>
					</s:else>

					<s:if test="%{pageBean.currentPage !=pageBean.totalPage}">
						<a
							href="<%=request.getContextPath()%>/Tcr_listAll.action?currentPage=${pageBean.currentPage+1}">下一页</a>
						<a
							href="<%=request.getContextPath()%>/Tcr_listAll.action?currentPage=${pageBean.totalPage}">最后一页</a>
					</s:if>
