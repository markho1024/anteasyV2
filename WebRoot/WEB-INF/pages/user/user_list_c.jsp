...
<script type="text/javascript">
	$(function() {
		$("#navReset").bind("click", function() {
			window.location.href = "<%=request.getContextPath()%>/UsrE_listC.action?soUser=";
		});
...
<script type="text/javascript">
	function doOff(id, name) {
		$.messager.confirm('锁定确认','你确认锁定" ' + name + ' "吗?',function(bt) {
			if (bt) {
				window.location.href = "<%=request.getContextPath()%>/UsrE_offC?id="+ id + "&offID=97";
							}
							;
						});
	}
 ...
 <div id="page">
	...
	<div id="context">
		...
		<div id="rightpane" class="right">
			...
			<div class="navSearch">
				<form action="UsrE_listC" id="navForm">
					<fieldset>
						<legend>
						...
						<table cellpadding="0" cellspacing="0" class="navSearch"
							...
							<tr>
								<th>用户类型</th>
								<td><p>
										<input type="text" id="USER_TYPE" name="soUser.userType"
					...
					<tbody>
						<s:iterator value="pageBean.list" status="st" id="bean">
							<tr <s:if test="!#st.odd">class="trodd"</s:if>>
								...
								<td><a
									href="<%=request.getContextPath()%>/UsrE_loadC.action?id=${bean.userID}">查看</a>
									<a
									href="<%=request.getContextPath()%>/UsrE_editC.action?id=${bean.userID}">修改</a>
